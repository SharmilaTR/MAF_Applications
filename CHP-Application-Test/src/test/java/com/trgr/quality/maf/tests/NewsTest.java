...
public class NewsTest extends BaseTest {
	 ...
	 */
	@Test(priority = 1, groups = { "chparg", "chpmex", "chpbr", "chppy", "chpury",
	 ...
	 */
	@Test(priority = 2, groups = { "chparg", "chpmex", "chpbr", "chppy", "chpury",
	 ...
	 */
	@Test(priority = 3, groups = { "chparg", "chpmex", "chpbr", "chppy", "chpury",
	 ...
	 */
	@Test(priority = 5, groups = { "chparg", "chppy", "chpmex", "chpbr", "chpury",
	...
	@Test(priority = 6, groups = { "chparg", "chppy", "chpury" }, description = "MAFQABANG-547")
	 ...
	 */
	@Test(priority = 7, groups = { "chppy", "chpury", "chparg", "chppe", "chpchile" }, description = "MAFQABANG-550")
	 ...
	 */
	@Test(priority = 9, groups = { "chppy", "chpury" }, description = "MAFQABANG-566")
	...
	/*
	 * Click on See IFRS news Link which will take user to Search Page then
	 * validate the content tree displayed on Search page then select any
	 * document from 2012 content tree link which will take user to doc display
	 * page then validate the Document display Page
	 */
	@Test(priority = 10, groups = { "chppy" }, description = "MAFQABANG-569")
	public void contentTreevaldiationforIFRSNewsLink() throws Exception {
		SoftAssert softas = new SoftAssert();
		try {
			testResult = Reporter.getCurrentTestResult();
			extentLogger = setUpExtentTest(extentLogger, "News", testResult.getMethod().getMethodName());
			boolean linkdisplayed, tocverified, docpageverified = false;

			String jiraNumber = testResult.getMethod().getDescription();
			String issueSummary = getIssueTitle(jiraNumber,
					"Verify content tree on Search page and document display when 'see IFRS News' is clicked");
			...
			for (Object searchString : listOfSearchData) {
				...
				String link = jsonReader.readKeyValueFromJsonObject(jsonObjectChild, linknameKey, extentLogger);
				String yearKey = "contenttree_year";
				String year = jsonReader.readKeyValueFromJsonObject(jsonObjectChild, yearKey, extentLogger);
				...
				newspage = homepage.ClickonNewsTab();
				linkdisplayed = newspage.isSeeIFRSNewsDispalyed();
				if (!linkdisplayed) {
					logExtentNoResultsAsInfo(extentLogger, issueSummary, linknameKey, link + " IFRS Link is not found",
							jiraNumber);
				}

				searchpage = newspage.clickIFRSNewsLink();
				tocverified = searchpage.isLastLinkDisplayedInContentTreeEquals(link);
				softas.assertTrue(tocverified, jiraNumber + ":" + "Tree of Contents validated in SearchPage");
				logExtentStatus(extentLogger, tocverified, "Tree of Contents validated in SearchPage", "Link Name ",
						link, jiraNumber);

				// Expand First level item in the tree structure
				WebElement treeElement = searchpage.getFirstLevelContentTree(year);
				...
				if (docDisplayedInTree) {
					Thread.sleep(2000);
					documentpage = searchpage.clickDocumentInSubContentTree(treeElement, document);
				}
				docpageverified = documentpage.isInformationClaveTabDisplayed()
						&& documentpage.isTreeOfContentTabDisplayed();

				softas.assertTrue(docpageverified, jiraNumber + ":" + issueSummary);
				logExtentStatus(extentLogger, docpageverified, issueSummary, "Link Name ", document, jiraNumber);
			}
		} catch (Exception exc) {
			logTestExceptionToExtentLogger(testResult.getMethod().getMethodName(), exc, extentLogger);
			softas.assertTrue(false, "Exception in Test");
	 ...
	 */
	@Test(priority = 11, groups = { "chparg", "chpmex", "chpbr", "chppy", "chpury",
	...
	@Test(priority = 12, groups = { "chppy" }, description = "MAFQABANG-575")
	public void dateVerificationinSearchResults() throws InterruptedException, IOException {
		SoftAssert softas = new SoftAssert();
		try {
			testResult = Reporter.getCurrentTestResult();
			extentLogger = setUpExtentTest(extentLogger, "News", testResult.getMethod().getMethodName());
			...
			boolean searchResultsDisplayed = false;
			boolean isExpectedDateDisplayedOnDoc = false;

			String jiraNumber = testResult.getMethod().getDescription();
			String issueSummary = getIssueTitle(jiraNumber, "Verifying date mentioned in Search results");

			// only reading this error message thats why didn't create json file
			// its time consuming
			String errmsg = "No se encontraron resultados";
			...
			for (Object searchString : listOfSearchData) {
				...
				String fromdateVal = jsonReader.readKeyValueFromJsonObject(jsonObjectChild, fromdateKey, extentLogger);
				String todateKey = "todate";
				String todateVal =jsonReader.readKeyValueFromJsonObject(jsonObjectChild, todateKey, extentLogger);;
				
				newspage = homepage.ClickonNewsTab();

				newspage.enterSinceDate(fromdateVal);
				newspage.enterUntildate(todateVal);
				...
				if(searchResultsDisplayed)
					newspage.isErrorMsgDisplayed(errmsg);

				if (errmsgDisplayed) {
					logExtentNoResultsAsInfo(extentLogger, issueSummary, "FromDate:" + fromdateVal, "SinceDate" +
							todateVal + " -resulted in no search results", jiraNumber);
					...
					isExpectedDateDisplayedOnDoc = documentpage.isDateofIssueFieldAvailable();
					if (isExpectedDateDisplayedOnDoc) {
						
						dateverified = documentpage.validateDateToBeInGivenRange(fromdateVal, todateVal, "Issue");
						softas.assertTrue(dateverified, jiraNumber + ":" + "Date on the document is in given range");
						logExtentStatus(extentLogger, dateverified, "Date on the document is in given range", jiraNumber);
											
					}
					else
					{
						logExtentNoResultsAsInfo(extentLogger, issueSummary, "DocumentDisplay page",
								"Date of Issue Field not available", jiraNumber);
						
						//Check for Date of Sanction in this case as either one of the dates should be available
						isExpectedDateDisplayedOnDoc = documentpage.isDateofSanctionIsFieldAvailable();
						softas.assertTrue(isExpectedDateDisplayedOnDoc, jiraNumber + ":" + "Date of sanction is displayed on document");
						logExtentStatus(extentLogger, isExpectedDateDisplayedOnDoc, "Date of sanction is displayed on document", jiraNumber);
						
						dateverified = documentpage.validateDateToBeInGivenRange(fromdateVal, todateVal, "Sanction");
						softas.assertTrue(dateverified, jiraNumber + ":" + "Date on the document is in given range");
						logExtentStatus(extentLogger, dateverified, "Date on the document is in given range", jiraNumber);
					}					
				}
